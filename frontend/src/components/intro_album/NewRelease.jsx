...
const NewRelease = () => {
  return (
    <div className="flex flex-col mt-20 items-center justify-center w-full overflow-hidden">
      ...
      <h1 className="text-white text-4xl font-bold">NEW RELEASE FOR YOU</h1>
      <div className="flex flex-row gap-4 my-20">
        {albums.new.map((album, index) => (
          <div
            key={index}
      ...
      <div className="flex flex-row justify-start text-white gap-36">
        <div className="flex flex-col gap-5">
          <h1 className="text-white text-2xl font-bold">MOST RECOMMENDED</h1>
          {albums.recommeded.map((album, index) => (
            <div key={index} className="flex items-center space-x-4 mb-6 group cursor-pointer">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-8">{album.year}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="flex flex-col gap-5">
          <h1 className="text-white text-2xl font-bold">MOST LISTENED</h1>
          {albums.most_listened.map((album, index) => (
            <div key={index} className="flex items-center space-x-4 mb-6 group cursor-pointer">
              <h3 className="font-bold">0{index + 1}</h3>
              <img
                src={album.image}
