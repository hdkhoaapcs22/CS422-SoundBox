...
const CreateAlbum = () => {
  ...
  const addCollaborator = (songId) => {
    setSongs((prevSongs) =>
      prevSongs.map((song) =>
        song.id === songId
          ? { ...song, collaborators: [...song.collaborators, ""] }
          : song
      )
    );
  };
          ...
          if (value && value.size > 0) {
            setTimeout(() => {
              setSongs((prev) => {
                ...
                refreshedSongs[songIndex].audioPreview = audioUrl;
  ...
  return (
    <div className="min-h-screen p-10 text-gray-300 overflow-y-auto">
      ...
      <form onSubmit={onSubmitHandler}>
        <div className="mb-4">
          ...
          <label htmlFor="uploadAlbumImage">
            <img
              ...
              src={
                albumImage ? URL.createObjectURL(albumImage) : assets.uploadArea
              }
              alt="Album Cover"
            />
            <input
        ...
        {songs.map((song) => (
          <div key={song.id} className="mb-6 border p-4 rounded bg-white/15">
            <div className="flex flex-row justify-between">
              <h3 className="text-2xl text-white font-semibold mb-4">Song</h3>
              <button
                type="button"
            ...
            <div className="flex gap-20 mb-4">
                ...
                <label htmlFor={`uploadSongImage-${song.id}`}>
                  <img
                    ...
                    src={
                      !song.image
                        ? assets.uploadArea
                        : URL.createObjectURL(song.image)
                    }
                    alt=""
                  />
                  <input
                    onChange={(e) =>
                      handleSongChange(song.id, "image", e.target.files[0])
                    }
                    type="file"
              ...
              <div>
                ...
                <input
                  ...
                  accept="audio/*"
                  onChange={(e) =>
                    handleSongChange(song.id, "audio", e.target.files[0])
                  }
                  required
                />
            ...
            <div className="mb-6">
              ...
              <select
                value={song.genre}
                onChange={(e) =>
                  handleSongChange(song.id, "genre", e.target.value)
                }
              ...
              {song.collaborators.map((collaborator, index) => (
                <div key={index} className="flex items-center gap-2 mb-2">
                  <input
                    ...
                    value={collaborator}
                    onChange={(e) =>
                      handleCollaboratorChange(song.id, index, e.target.value)
                    }
                  ...
                  <button
                    type="button"
                    onClick={() => removeCollaborator(song.id, index)}
